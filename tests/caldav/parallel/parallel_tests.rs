...
fn generate_unique_calendar_name() -> String {
    format!("parallel_test_calendar_{}", chrono::Utc::now().timestamp_millis())
}

/// Helper function to generate unique event UIDs
fn generate_unique_event_uid() -> String {
    format!("parallel-event-{}@example.com", chrono::Utc::now().timestamp_millis())
}
...
<D:propfind xmlns:D="DAV:">
  <D:prop>
    ...
    let results = client.propfind_many(paths, Depth::Zero, propfind_body, 2).await;
    ...
    for (i, result) in results.into_iter().enumerate() {
        match result.result {
            Ok(response) => {
                println!("PROPFIND {} ({}): {}", i, result.pub_path, response.status());
                assert!(response.status().is_success(), "Expected successful PROPFIND for {}", result.pub_path);
            }
            Err(e) => {
...
<C:mkcalendar xmlns:D="DAV:" xmlns:C="urn:ietf:params:xml:ns:caldav">
  <D:set>
    ...
    if !calendar_paths.is_empty() {
        ...
        let report_body = Arc::new(Bytes::from(r#"<?xml version="1.0" encoding="utf-8"?>
<D:sync-collection xmlns:D="DAV:">
  ...
  <D:prop>
    <D:getetag/>
        ...
        let results = client.report_many(calendar_paths, Depth::Zero, report_body, 3).await;
            ...
            match result.result {
                Ok(response) => {
                    ...
                    assert!(response.status().is_success() || response.status().is_client_error(), 
                            "Expected successful or client error REPORT for {}", result.pub_path);
                }
                Err(e) => {
                    println!("REPORT {} ({}) failed (may be expected): {}", i, result.pub_path, e);
                }
            }
        }
    }
...
END:VCALENDAR"#, event_uid, i, i);
        ...
        match result {
            Ok(Ok(response)) => {
                if response.status().is_success() {
                    ...
                    println!("Parallel event creation #{} failed: {}", i + 1, response.status());
                }
            }
            Ok(Err(e)) => {
    ...
    println!("Successfully created {}/5 events in parallel", success_count);
    assert!(success_count >= 3, "Expected at least 3 successful parallel creations");
