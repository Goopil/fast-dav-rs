...
<C:mkcalendar xmlns:D="DAV:" xmlns:C="urn:ietf:params:xml:ns:caldav">
  <D:set>
    ...
    assert!(
        put_response.unwrap().status().is_success(),
    ...
    match copy_response {
        Ok(resp) => {
            ...
            assert!(
                ...
                "Expected successful COPY operation, got status: {}",
                resp.status()
            );
        }
        Err(e) => {
    ...
    let move_response = client
        .r#move(&event_path, &full_moved_event_url, true)
        .await;
    match move_response {
        Ok(resp) => {
            ...
            assert!(
                resp.status().is_success() || resp.status() == hyper::StatusCode::FORBIDDEN,
                "Expected successful MOVE operation or FORBIDDEN, got status: {}",
                resp.status()
            );
        }
        Err(e) => {
