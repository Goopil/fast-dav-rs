...
<C:mkcalendar xmlns:D="DAV:" xmlns:C="urn:ietf:params:xml:ns:caldav">
  <D:set>
    ...
    let verify_response = client.propfind(&calendar_path, fast_dav_rs::Depth::Zero, 
  ...
  <D:set>
    ...
    match proppatch_response {
        Ok(resp) => {
            ...
            assert!(resp.status().is_success(), "Expected successful PROPPATCH operation, got status: {}", resp.status());
        }
        Err(e) => {
...
<D:propfind xmlns:D="DAV:">
  <D:prop>
    ...
    match verify_response {
        Ok(resp) => {
            assert!(resp.status().is_success(), "Expected successful verification PROPFIND, got status: {}", resp.status());
        }
        Err(e) => {
