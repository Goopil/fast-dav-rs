...
<D:propfind xmlns:D="DAV:">
  <D:prop>
    ...
    // Test Depth::Zero
    let zero_result = client
        .propfind("calendars/test/", Depth::Zero, propfind_body)
        .await;
    match zero_result {
        Ok(response) => {
    ...
    // Test Depth::One
    let one_result = client
        .propfind("calendars/test/", Depth::One, propfind_body)
        .await;
    match one_result {
        Ok(response) => {
    ...
    match mk_response {
        Ok(response) => {
            ...
            if !response.status().is_success() {
                println!(
                    "⚠️  MKCALENDAR with standard namespaces failed (may be expected with this server)"
                );
            }
        }
        Err(e) => {
...
<C:mkcalendar xmlns:D="DAV:" xmlns:C="urn:ietf:params:xml:ns:caldav">
  <D:set>
    ...
    match mk_response {
        ...
        Err(e) => {
            println!(
                "MKCALENDAR with special characters failed (may be expected): {}",
                e
            );
        }
    }
...
END:VCALENDAR"#,
    ...
    match query_result {
        Ok(objects) => {
            println!(
                "Calendar query with UTC format returned {} objects",
                objects.len()
            );
...
END:VCALENDAR"#,
        ...
        let conditional_put = client
            .put_if_match(&event_path, Bytes::from(updated_event_ics), &etag_value)
            .await;
