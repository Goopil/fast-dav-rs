...
fn generate_unique_calendar_name() -> String {
    format!("resilience_test_calendar_{}", chrono::Utc::now().timestamp_millis())
}

/// Helper function to generate unique event UIDs
fn generate_unique_event_uid() -> String {
    format!("resilience-event-{}@example.com", chrono::Utc::now().timestamp_millis())
}
...
async fn test_resilience_network_disconnect() {
    ...
    match options_result {
        ...
        Err(e) => {
            println!("⚠️ Initial connection failed (maybe expected in some environments): {}", e);
    ...
    assert!(success_count >= 7, "Expected at least 7 successful rapid requests");
}
...
async fn test_resilience_high_concurrency_load() {
    ...
    let expanded_paths: Vec<String> = paths.into_iter()
        .cycle()
        .take(8) // Reasonable number of requests
        .collect();
    
    let propfind_body = Arc::new(Bytes::from(r#"<?xml version="1.0" encoding="utf-8"?>
<D:propfind xmlns:D="DAV:">
  <D:prop>
    ...
    let results = client.propfind_many(expanded_paths, Depth::Zero, propfind_body, 4).await;
    
    let results_len = results.len();
    println!("High concurrency test completed with {} results", results_len);
    ...
    for (i, result) in results.into_iter().enumerate() {
        match result.result {
            Ok(response) => {
                ...
                println!("High concurrency request {} ({}): {}", i, result.pub_path, response.status());
            }
            Err(e) => {
                println!("High concurrency request {} ({}) failed: {}", i, result.pub_path, e);
            }
        }
    }
    
    println!("Successful high concurrency requests: {}/{}", success_count, results_len);
    // Expect at least 75% success rate
    assert!(success_count >= (results_len * 3 / 4), 
...
<C:mkcalendar xmlns:D="DAV:" xmlns:C="urn:ietf:params:xml:ns:caldav">
  <D:set>
    ...
    if let Err(e) = mk_response {
        println!("⚠️ Failed to create a calendar for a large payload test: {}", e);
        return;
    }
...
END:VCALENDAR"#, event_uid, large_description);
    ...
    match get_response {
        Ok(response) => {
            assert!(response.status().is_success(), "Expected successful large payload GET");
...
END:VCALENDAR"#, event_uid, i, i);
    ...
    if !event_hrefs.is_empty() {
        ...
        let multiget_results = client.calendar_multiget(&calendar_path, &event_hrefs, true).await;
        match multiget_results {
            Ok(objects) => {
                ...
                if objects.len() > 0 {
                    ...
                    let objects_with_data = objects.iter()
                        .filter(|obj| obj.calendar_data.is_some())
                        .count();
                    
                    println!("Objects with calendar data: {}", objects_with_data);
                } else {
                    println!("⚠️  Calendar multiget returned no objects (may be expected with this server)");
                }
            }
            Err(e) => {
                println!("⚠️  Calendar multiget failed (may be expected with this server): {}", e);
            }
        }
    } else {
