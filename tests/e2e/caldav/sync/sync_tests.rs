...
fn generate_unique_calendar_name() -> String {
    format!("sync_test_calendar_{}", chrono::Utc::now().timestamp_millis())
}

/// Helper function to generate unique event UIDs
fn generate_unique_event_uid() -> String {
    format!("sync-event-{}@example.com", chrono::Utc::now().timestamp_millis())
}
...
END:VCALENDAR"#, event_uid, i, i);
    ...
    match sync_response {
        Ok(response) => {
            ...
            assert!(response.items.len() >= event_paths.len(), 
                "Expected at least {} items, got {}", event_paths.len(), response.items.len());

            // Verify we got calendar data for events
            let event_items: Vec<_> = response.items.iter()
                .filter(|item| item.calendar_data.is_some())
                .collect();

            println!("Events with data: {}", event_items.len());
            assert!(event_items.len() >= event_paths.len(), 
                "Expected calendar data for at least {} events", event_paths.len());
        }
        Err(e) => {
...
<C:mkcalendar xmlns:D="DAV:" xmlns:C="urn:ietf:params:xml:ns:caldav">
  <D:set>
    ...
    let initial_sync = client.sync_collection(&calendar_path, None, Some(100), true).await;
    let initial_sync_token = match initial_sync {
        Ok(response) => {
            println!("Initial sync baseline established with {} items", response.items.len());
            ...
            if response.sync_token.is_none() {
                println!("ℹ️  No sync token received from server - incremental sync testing will be limited");
            }
...
END:VCALENDAR"#, event_uid);
    ...
    let incremental_sync = client.sync_collection(&calendar_path, Some(&token), Some(100), true).await;

    match incremental_sync {
        Ok(response) => {
            println!("Incremental sync completed with {} items", response.items.len());
            println!("New sync token: {:?}", response.sync_token);

            // Find the new event in the sync response
            let new_event_item = response.items.iter()
                .find(|item| item.href.contains(&event_uid));

            if let Some(item) = new_event_item {
                println!("✅ Found new event in sync: {} (ETag: {:?})", item.href, item.etag);
    ...
    match delete_response {
        Ok(resp) => {
            assert!(resp.status().is_success(), "Expected successful event deletion");
    ...
    let pre_delete_sync = client.sync_collection(&calendar_path, initial_sync_token.as_deref(), Some(100), true).await;
    ...
    if let Some(ref_token) = pre_delete_token {
        ...
        let deletion_sync = client.sync_collection(&calendar_path, Some(&ref_token), Some(100), true).await;
        
        match deletion_sync {
            Ok(response) => {
                println!("Deletion sync completed with {} items", response.items.len());
                
                // Check if we have deletion markers
                let deleted_items: Vec<_> = response.items.iter()
                ...
                if !deleted_items.is_empty() {
                    ...
                    for item in deleted_items {
                        ...
                        assert!(item.calendar_data.is_none(), "Deleted items should have no calendar data");
                    }
                } else {
...
END:VCALENDAR"#, event_uid, i, i);
    ...
    let limited_sync = client.sync_collection(&calendar_path, None, Some(3), true).await;
    match limited_sync {
        Ok(response) => {
            println!("Limited sync (3) completed with {} items", response.items.len());
            ...
            if response.items.len() > 3 {
                println!("⚠️  Server returned more items ({}) than requested limit (3)", response.items.len());
            } else {
    ...
    let unlimited_sync = client.sync_collection(&calendar_path, None, Some(10), true).await;
    match unlimited_sync {
        Ok(response) => {
            println!("Unlimited sync (10) completed with {} items", response.items.len());
            
            // Should have at least our 5 items
            assert!(response.items.len() >= event_uids.len(), 
                "Expected at least {} items, got {}", event_uids.len(), response.items.len());
            
            // Validate that all our events are present
            let mut found_events = 0;
            for event_uid in &event_uids {
                let event_found = response.items.iter()
            ...
            assert_eq!(found_events, event_uids.len(), 
                "Expected to find all {} events, found {}", event_uids.len(), found_events);
        }
        Err(e) => {
...
END:VCALENDAR"#, event_uid);
    ...
    let initial_sync_token = match initial_sync {
        Ok(response) => {
            ...
            let event_found = response.items.iter()
                .any(|item| item.href.contains(&event_uid));
            assert!(event_found, "Expected to find our test event in initial sync");
    ...
    if let Some(token) = initial_sync_token {
        let deletion_sync = client.sync_collection(&calendar_path, Some(&token), Some(100), true).await;
        match deletion_sync {
            Ok(response) => {
                println!("Deletion sync completed with {} items", response.items.len());
                
                // Should have at least one item (the deleted event)
                assert!(!response.items.is_empty(), "Expected at least one item in deletion sync");
