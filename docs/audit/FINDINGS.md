...
- **Status:** ✅ Partially fixed 2026-09-02 (v1.0 waves + v1.1 e2e gate, issue #124): wire-level request/response fakes (`tests/unit/common/http_helpers.rs`) and transport e2e (`tests/e2e/caldav/core/transport_tests.rs` — chunked reassembly through the real stack, Basic/Bearer auth on the wire, HTTP/1.1 honesty check) close the wire-coverage items; the v1.1 e2e gate adds live-server smoke for the Auto request-compression probe on a real PUT (AUDIT-012 fix) and `with_hyper_client` injection, and hardens the parallel-suite soft asserts (swallowed creation errors and success-or-client-error REPORT assertions now panic). **Still open:** connection-reuse/keep-alive proof and a `tokio::time::pause` client-level timeout test (low value per audit; parking_lot removed the poisoning class).
