...
5. **AUDIT-022** — replace the 4 production `unwrap`/`expect` in `webdav/client.rs` (`Method::from_bytes` ×2, `acquire_owned().expect`, `HeaderValue::from_str().unwrap`) with `Error::other` typed errors. Mechanical, no behavior change.
